...
export default function TechnicianDashboardErrorBoundary({ error, reset }: ErrorProps) {
  useEffect(() => {
    ...
    <div className="flex flex-col items-center justify-center min-h-[40vh] p-8 text-center bg-slate-50/50 dark:bg-slate-950/20 rounded-3xl border border-slate-200/60 dark:border-slate-800/60">
      <div className="max-w-md space-y-5">
        ...
        <div className="space-y-2">
          <h2 className="text-lg font-bold text-slate-800 dark:text-white">Technician Dashboard Error</h2>
          <p className="text-xs text-slate-400 font-medium">
            An error occurred while loading this dashboard page. The technician navigation layout remains fully active.
          </p>
        </div>
        {error.message && (
